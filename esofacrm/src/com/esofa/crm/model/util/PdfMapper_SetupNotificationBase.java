...
@Component
public class PdfMapper_SetupNotificationBase extends PdfMapper<Long> {
	...
	public void mapFields(AcroFields fields, Long itemId) throws IOException, DocumentException { 
		...
		fields.setField("Health Card", customer.getFullHealthCardNumber());
		...
		if( custMedInfo != null) {
			...
			if (sleepClinic != null) {
				fields.setField("sleepClinic",sleepClinic.getName());
			}
			
			if (sleepDoctor !=null) {
				fields.setField("sleepDoc",sleepDoctor.getName());
			}
		}
		...
		fields.setField("Date of setup",EsofaUtils.getDateFormatted(new Date()));
		...
		if (customer.getPreferredLocation() !=null) {
			...
			if (s !=null) {
				...
				fields.setField("shopAddress",shopAddress.toString());
			}
		}
		...
		if (customer.getClinician() !=null) {
			...
			if (clinician !=null) {
				fields.setField("clinician",clinician.getFirstname() + " "  + clinician.getLastname());
			}
		}
